...
@section('title', 'Daftar CCTV - TMC Balikpapan')
@section('page-title', 'Daftar CCTV')
...
<div class="px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Kelola semua CCTV yang terdaftar dalam sistem.
            </p>
        </div>
        <div class="mt-5 sm:mt-0 sm:ml-6">
            <a href="{{ url('/admin/cctvs/create') }}" 
    ...
    <div class="mt-6 bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form method="GET" action="{{ url('/admin/cctvs') }}">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $cctvs->firstItem() + $index }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $cctv->name }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ $cctv->location->name ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                {{ $cctv->camera_type == 'PTZ' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                                {{ $cctv->camera_type }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ url('/admin/cctvs/' . $cctv->id . '/edit') }}" 
                                ...
                                <form action="{{ url('/admin/cctvs/' . $cctv->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="text-red-600 hover:text-red-900 p-2 rounded-md hover:bg-red-50 transition-colors"
                                            onclick="return confirm('Apakah Anda yakin ingin menghapus CCTV ini?')">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Menampilkan
                        <span class="font-medium">{{ $cctvs->firstItem() }}</span>
                        sampai
                        <span class="font-medium">{{ $cctvs->lastItem() }}</span>
                        dari
                        <span class="font-medium">{{ $cctvs->total() }}</span>
                        hasil
                    </p>
                </div>
                <div>
